...
data = [list(row.values()) for row in dataBase.getRedTeamData()]
table_frame = None
...
def createRedTableFrame(parent, team_name, background_color, border_color):
    ...
    # Create labels and place headers to the table
    headers = ["ID", "First Name", "Last Name", "Codename"]
    for j, header in enumerate(headers):
        label = customtkinter.CTkLabel(table_frame, text=header, font=("Impact", 15), text_color="whitesmoke")
...
def update_table():
    ...
    newData = [list(row.values()) for row in dataBase.getRedTeamData()]

    # Add new data to the table
    for i, data_row in enumerate(newData, start=2):
        for j, cell_data in enumerate(data_row):
            label = customtkinter.CTkLabel(table_frame, text=cell_data, font=("Arial", 10), text_color="whitesmoke")
